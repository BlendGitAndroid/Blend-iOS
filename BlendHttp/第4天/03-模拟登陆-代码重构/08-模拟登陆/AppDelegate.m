...
- (BOOL)application:(UIApplication *)application
    ...
    // 初始化window对象
    // 手动创建和设置应用程序的主窗口，因为没有设置主窗口，所以程序会黑屏
    self.window = [[UIWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
    [self.window makeKeyAndVisible];

    // 如果有账号和密码
    if ([HMNetworkTool sharedNetworkTool].name ||
        [HMNetworkTool sharedNetworkTool].pwd) {
        [[HMNetworkTool sharedNetworkTool]
            loginWithSuccess:^{
        ...
        // 设置过渡界面，在Login的sb中有一个标识为“guodu”的界面
