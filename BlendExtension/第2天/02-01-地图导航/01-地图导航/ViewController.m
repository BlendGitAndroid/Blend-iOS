...
- (IBAction)navigateClick:(id)sender {
    ...
    // 2. 调用地理编码方法
    [geocoder
        geocodeAddressString:self.destinationTF.text
           completionHandler:^(NSArray<CLPlacemark *> *_Nullable placemarks,
             ...
             for (CLPlacemark *pm in placemarks) {
                 ...
                 // 6. 创建一个MKMapItem
                 MKMapItem *destItem =
                     [[MKMapItem alloc] initWithPlacemark:mkpm];
...
- (void)test {
    ...
    // MKPlacemark *mkpm = [MKPlacemark alloc] initWithPlacemark:<#(nonnull
    // CLPlacemark *)#>

    // 创建一个MKMapItem --> 终点的位置
    // MKMapItem *destinationItem = [MKMapItem alloc]
    // initWithPlacemark:<#(nonnull MKPlacemark *)#>

    // MKMapItem : 地图上的一个点
    // openMapsWithItems : 调用此方法就可以打开系统自带地图导航
    //[MKMapItem openMapsWithItems:<#(nonnull NSArray<MKMapItem *> *)#>
    //launchOptions:<#(nullable NSDictionary<NSString *,id> *)#>]
}
