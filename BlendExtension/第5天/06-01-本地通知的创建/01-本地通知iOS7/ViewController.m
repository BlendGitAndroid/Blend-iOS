...
/**
 ...
 // 设置图标的数字角标 (图标标记)
 @property(nonatomic) NSInteger applicationIconBadgeNumber;  // 0 means no
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    // 2.1 设置触发时间， 5秒后触发
    localNotifi.fireDate = [NSDate dateWithTimeIntervalSinceNow:5];
    ...
    // 2.4 设置 默认YES
    // 是否显示锁屏时的文字以及提醒样式的按钮文字，已经被弃用
    localNotifi.hasAction = YES;

    // 2.5 设置 提醒样式的按钮文字 / 锁屏界面底部的文字，已经被弃用
    ...
    // localNotifi.repeatCalendar =
    //     [NSCalendar calendarWithIdentifier:
    //                     @"跳进官方文档, 里面就有一堆的标示符,找chinese的选项, "
    //                     @"就代表是农历"];

    // 3. 调度通知 , 如果是IOS7 , 这一步写完就OK
    // schedule : 调度,将通知加入到本地调度池中
    ...
    // 获取通知 --> 配合删除用的
    // scheduledLocalNotifications 获取当前程序注册的所有通知
