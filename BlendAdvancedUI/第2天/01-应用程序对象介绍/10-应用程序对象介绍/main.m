...
/**
 应用启动过程:
    ...
    6>  将应用程序代理对象内的window实例化,并且设置为应用程序的keyWindow[主窗口].iOS系统会自动创建UIWindow对象 并赋值给AppDelegate的window属性。
    7>  iOS通过Info.plist中的 UIMainStoryboardFile（也就是Main storyboard file base name属性） 配置实现了"零代码"的窗口和控制器加载机制，这是Storyboard开发模式的核心优势之一。这种方式简化了应用程序的启动流程，使开发者可以更多地专注于界面设计和业务逻辑。
    最后加载配置文件中指定的storyboard[Main.storyboard]文件中带箭头的控制器。
 */
