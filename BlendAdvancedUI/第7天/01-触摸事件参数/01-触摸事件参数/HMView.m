...
- (void)touchesBegan:(NSSet<UITouch*>*)touches withEvent:(UIEvent*)event
{
    ...
    // 获取触摸对象
    // .anyObject 是NSSet集合的一个方法，用于获取集合中的任意一个对象
    // 整行代码的作用是从触摸事件的集合中获取一个触摸对象，并将其赋值给变量 t
...
- (void)touchesMoved:(NSSet<UITouch*>*)touches withEvent:(UIEvent*)event
{
    ...
    // 上一个点的位置
    // 基本功能
    // - 该方法返回一个CGPoint类型的值，表示触摸点在上一次事件更新时的坐标位置
    // - 坐标值是相对于传入的视图(view)坐标系的，也就是这个点在传入视图的坐标位置
