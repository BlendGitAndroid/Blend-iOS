/*
  ...
  1. 野指针
     ...
     注意区分C语音的野指针和OC的野指针，C语音的就是指没有初始化被随机指向了一个垃圾值，而OC的就是被指向的对象被回收了。
 ...
 3. 僵尸对象
    ...
    僵尸对象可以理解为，使用一个已经被释放的对象的，可能有问题，也可坑没有问题
 ...
 野指针 → 指向无效内存的指针（可能读到垃圾值或崩溃）。

 僵尸对象 → 调试工具故意保留的已释放对象（强制报错帮助调试）。

 安全做法：

 MRC：释放后立即置空指针。

 ARC：避免使用 unsafe_unretained（类似野指针）。
...
//    [p1 release];//0 对象，这里release后会立即被回收，这个时候的p1就是野指针
//    
//    
//    [p1 retain]; //1
